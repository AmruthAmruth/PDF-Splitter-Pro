...
const applyPublicDnsFallback = (): void => {
  try {
...
export const connectDB = async (): Promise<void> => {
  ...
  try {
    ...
    if (mongoURI.startsWith("mongodb+srv://") && message.includes("querySrv ECONNREFUSED")) {
      console.error(
        "MongoDB Atlas SRV DNS lookup failed. Attempting a retry using public DNS servers."
      );
      ...
      try {
        ...
        console.error(
          `MongoDB Retry Connection Error: ${(retryError as Error).message}`
        );
      }
    }
